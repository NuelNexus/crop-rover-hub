...
type WalleAvatarProps = {
  className?: string;
  size?: number;
};

const WalleAvatar = ({ className = "", size = 32 }: WalleAvatarProps) => {
  const uniqueId = useId();
  const clip1 = `${uniqueId}-clip-1`;
  const clip2 = `${uniqueId}-clip-2`;
  const clip3 = `${uniqueId}-clip-3`;
  const clip4 = `${uniqueId}-clip-4`;
  const clip5 = `${uniqueId}-clip-5`;

  return (
    <svg
      className={`walle-avatar ${className}`.trim()}
